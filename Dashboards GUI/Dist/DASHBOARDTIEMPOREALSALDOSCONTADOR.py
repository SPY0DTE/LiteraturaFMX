...
def update_time():
    ...
    try:
        RawDashboard=pd.read_excel(Direccion+"\\UCITS\\"+Blotter) #Abrimos el Excel donde llevo mis operaciones, Como estamos trabajando con un programa recurrente aveces cuando guardo el archivo este coincide con la ejecucion del programa lo que hace que se rompa
    except:
        time.sleep(5)
        RawDashboard=pd.read_excel(Direccion+"\\UCITS\\"+Blotter) #Abrimos el Excel donde llevo mis operaciones, Como estamos trabajando con un programa recurrente aveces cuando guardo el archivo este coincide con la ejecucion del programa lo que hace que se rompa
    ...
    ComprasTotalesArbitrajeMexico=ComprasArbitrajes["Operado Asigna"].sum()
    VentasArbitrajes=Arbitrajes[Arbitrajes["Sentido"]=="VENTA"]
    VentasTotalesArbitrajesMexico=VentasArbitrajes["Operado Asigna"].sum()
